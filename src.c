...
{
    select_result * t=NULL;
    t=(select_result *)malloc(4001*sizeof(select_result));
    int x,y,z;
    int n=0;
    for(x=0;x<100;x++)
    {
        if(strcmp(cus[x].c_mkgsegment,mktsegment)==0)
        {
            for(y=0;y<4000;y++)
            {
                if(cus[x].c_custkey==ord[y].o_custkey&&strcmp(ord[y].o_orderdate,order_date)<0)
                {
                    for(z=0;z<1000;z++)
                    {
                        if(ord[y].o_orderkey==item[z].l_orderkey&&strcmp(item[z].l_shipdate,ship_date)>0)
                        {
                            t[n].l_extendedprice=item[z].l_extendedprice;
                            strcpy(t[n].o_orderdate,ord[y].o_orderdate);
                            t[n].l_orderkey=item[z].l_orderkey;
                            if(n>0 && t[n-1].l_orderkey==t[n].l_orderkey)
                            {
                                t[n-1].l_extendedprice+=t[n].l_extendedprice;
                                n--;
                            }
                            n++;
                        }
...
int main(int argc,char * argv[])//argc表示输入内容的总个数，argv[]内保存着输入的内容
{
	...
	strcpy(mktsegment,argv[5+i]);
		...
		result=Select(cus,ord,item,argv[6+i],ship_date,mktsegment);
